...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //웹뷰 설정을 위한 WebSettings 객체 생성
        WebSettings settings = webView.getSettings();
        settings.setJavaScriptEnabled(true); //Javascript의 사용여부 설정
    ...
    /*
    웹뷰 세팅을 위한 클래스정의2
        : WebViewClient 클래스를 상속하여 정의한다. Javascript의 alert()를
        앱에 맞게 Toast로 변경하기 위한 클래스로 웹에서 사용하는 alert()의
        기본창을 사용하지 않는다.
        또한 경고창을 별도로 띄우지 않고 파라미터를 전달하는 용도로도 사용된다.

        자바스크립트의 alert를 적절히 처리하기 위해 개발자정의의
     */
    class CustomWebChromeClient extends WebChromeClient{
        ...
        public boolean onJsAlert(WebView view, String url, String message, JsResult result) {
            ...
            /*
            1|안녕하세요|kosmo
                : alert()로 위와 같이 전달되는 경우 split()으로 아래와 같이 문자열 배열로 만들어
            ...
            if(msgArray[0].equals("1")){
                //파싱한 결과 첫번째 값이 1인 경우 파라미터로 판단하여 Logcat에 출력한다.
                Log.d("KOSMO123", "파라미터가 전달되었습니다.");
                Log.d("KOSMO123", "파라미터1="+msgArray[1]);
                Log.d("KOSMO123", "파라미터2="+msgArray[2]);
            }
            else{
                //만약 1이 아닌경우 일반 알림으로 판단하여 토스트로 변환한다.
                Toast.makeText(MainActivity.this, message, Toast.LENGTH_LONG).show();
            }

            //alert()가 뜨면 확인버튼을 눌러서 닫아줘야 한다. 근데 안드로이드는 안뜨니까 아래의 코드를 통해서 자동으로 닫아준다.
            /*
            아래와 가이 처리하면 alert()창을 확인버튼을 누른 것으로 간주하여
